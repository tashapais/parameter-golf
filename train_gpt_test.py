...
# Using PyTorch SDPA as flash_attn fallback for CUDA 12.4 compatibility

def flash_attn_3_func(q, k, v, causal=True, **kwargs):
    # q,k,v: (batch, seq, heads, head_dim) -> need (batch, heads, seq, head_dim)
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
def flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, causal=True, **kwargs):
    # q,k,v: (total_seqlen, heads, head_dim) - varlen not easily supported without flash_attn
    # Fall back to padded batch processing
    seqlens = (cu_seqlens_q[1:] - cu_seqlens_q[:-1]).tolist()
    batch_size = len(seqlens)
    # Pad to max_seqlen
    q_padded = torch.zeros(batch_size, max_seqlen_q, q.shape[1], q.shape[2], device=q.device, dtype=q.dtype)
    k_padded = torch.zeros(batch_size, max_seqlen_q, k.shape[1], k.shape[2], device=k.device, dtype=k.dtype)
    v_padded = torch.zeros_like(k_padded)
    for i, slen in enumerate(seqlens):
        start = cu_seqlens_q[i].item()
        q_padded[i, :slen] = q[start:start+slen]
        k_padded[i, :slen] = k[start:start+slen]
        v_padded[i, :slen] = v[start:start+slen]
    # (batch, seq, heads, head_dim) -> (batch, heads, seq, head_dim)
    q_t = q_padded.transpose(1, 2)
    k_t = k_padded.transpose(1, 2)
    v_t = v_padded.transpose(1, 2)
    if k_t.shape[1] != q_t.shape[1]:
        reps = q_t.shape[1] // k_t.shape[1]
        k_t = k_t.repeat_interleave(reps, dim=1)
        v_t = v_t.repeat_interleave(reps, dim=1)
    out = torch.nn.functional.scaled_dot_product_attention(q_t, k_t, v_t, is_causal=causal)
    out = out.transpose(1, 2)  # (batch, seq, heads, head_dim)
    # Unpad back to (total_seqlen, heads, head_dim)
    result = torch.zeros_like(q)
    for i, slen in enumerate(seqlens):
        start = cu_seqlens_q[i].item()
        result[start:start+slen] = out[i, :slen]
    return result
from concurrent.futures import ThreadPoolExecutor
import triton
import triton.language as tl
# TensorDescriptor removed (using PyTorch MLP fallback)
...
def _leaky_relu_square(x):
    """LeakyReLU(0.5)^2 activation (fallback PyTorch impl for torch.compile compatibility)."""
    lr = torch.where(x > 0, x, 0.5 * x)
    return lr * lr


def FusedLeakyReLUSquareMLP(x, w1, w2):
    h = F.linear(x.reshape(-1, x.shape[-1]), w1)
    h = _leaky_relu_square(h)
    out = F.linear(h, w2)
...
def train_model(h, device, val_data):
    ...
    compiled_model = base_model  # torch.compile disabled for 1xH100 compatibility
    compiled_forward_logits = base_model.forward_logits  # torch.compile disabled
...
def train_and_eval(h, device):
    ...
    compiled_model = eval_model  # torch.compile disabled
    compiled_forward_logits = eval_model.forward_logits  # torch.compile disabled
    timed_eval(
    ...
    if h.ttt_enabled:
        ...
        def _fwd_ttt(input_ids, target_ids, lora):
            ...
            if _fwd_ttt_compiled_inner is None:
                _fwd_ttt_compiled_inner = _fwd_ttt_inner  # torch.compile disabled
